...
function hh_profile_editor_enqueue_scripts(){
    ...
    wp_register_script( 'hh_ajax_fe_profile-script', plugin_dir_url( __FILE__ ) . 'inc/js/profile-edit.js', array('jquery'),'a05');
    if (!is_admin()) {
...
function hh_init_imageCropper_plugin() {
    $cropPath = plugin_dir_url( __FILE__ ) . 'inc/vendor/imagecrop/';
    wp_register_style( 'hh_ajax_crop-style',  $cropPath . 'crop.css' );
    wp_register_script( 'hh_ajax_crop-script',  $cropPath . 'crop.min.js', array('jquery'),'a05');
    if (!is_admin()) {
        wp_enqueue_script( 'hh_ajax_crop-script' );
        wp_enqueue_style( 'hh_ajax_crop-style' );
    }
}
add_action('init', 'hh_init_imageCropper_plugin');
